...
static const esp_gatts_attr_db_t gatt_db[MIDI_IDX_NB] = {
    ...
    [IDX_SVC]
    = {{ESP_GATT_AUTO_RSP},
       {ESP_UUID_LEN_16, (uint8_t *)&primary_service_uuid, ESP_GATT_PERM_READ, sizeof(service_uuid), sizeof(service_uuid), service_uuid}},

    // MIDI Characteristic Declaration
    [IDX_MIDI_CHAR] = {{ESP_GATT_AUTO_RSP},
                       {ESP_UUID_LEN_16,
                        (uint8_t *)&character_declaration_uuid,
                        ESP_GATT_PERM_READ,
                        sizeof(char_prop_read_write_notify),
                        sizeof(char_prop_read_write_notify),
                        (uint8_t *)&char_prop_read_write_notify}},

    // MIDI Characteristic Value
    [IDX_MIDI_CHAR_VAL] = {{ESP_GATT_AUTO_RSP},
                           {ESP_UUID_LEN_128,
                            midi_char_uuid,
                            ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
                            CHAR_VAL_LEN_MAX,
                            sizeof(empty_midi_packet),
                            (uint8_t *)empty_midi_packet}},

    // MIDI Client Characteristic Configuration Descriptor
    [IDX_MIDI_CHAR_CFG] = {{ESP_GATT_AUTO_RSP},
                           {ESP_UUID_LEN_16,
...
static void gatts_profile_midi_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t * param)
{
    switch (event)
    {
        ...
        case ESP_GATTS_CONNECT_EVT:
            ...
            xTaskCreate(send_notes, "notes sender", 2048, (void *)task, tskIDLE_PRIORITY, NULL);
            ...
            else if (param->add_attr_tab.num_handle != MIDI_IDX_NB)
            {
                ESP_LOGE(
                    ...
                    "create attribute table abnormally, num_handle (%d) \
                        doesn't equal to MIDI_IDX_NB(%d)",
